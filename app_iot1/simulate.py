...
#
#   Changelog           :   Modified from original from previous blog that posted to Mongo to now to post to
#
...
def run_simulation(site, current_time, config_params):
    ...
    topic     = config_params["KAFKA_TOPIC"], 
    ...
    if site["data_persistence"] > 0:
        ...
        connection_saver, json_serializer = connection.createConnectionToStore(config_params, site, logger)      
        ...
        if  json_serializer is None:
            logger.critical(f"SiteId: {str(site["siteId"])} - createConnectionToStore.json_serializer Failed, exiting.")
            os._exit(1)
                        
        # end if
        
    # ok, lets proceed with production
            ...
            for device in site["devices"]:
                for sensor in device["sensors"]:
                    ...
                    if site["data_persistence"] > 0:  # Lets save it...
                        if mode == 1:   # save One
                            result              = connection.savePayloadToStore(connection_saver, json_serializer, site, mode, payload, topic, logger)
                            batch_flush_counter = 0
                            ...
                            if batch_flush_counter==flush_size:  # Post and Flush...
                                ...
                                result = connection.savePayloadToStore(connection_saver, json_serializer, site, mode, payloadset, topic, logger)
